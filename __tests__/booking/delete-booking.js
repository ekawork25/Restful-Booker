const bookingCollection = require('../../collections/booking/delete-booking');
...
describe('test delete booking', () => {
    test.each(Object.values(testData))(
        "Test $title ", async ({ title, header, body, id, expected_result }) => {

            if (title.includes('N, delete booking with invalid token')){
                header["Cookie"] = 'token=abc123';
            }
            else if(title.includes('N, delete booking without cookie in header')){
                header["Cookie"] = null;
            }
            else{
            ...
            res = await bookingCollection.deleteBooking(header, id, token);
